...
def getSF2Mu(lep1pt, lep1eta, lep2pt, lep2eta, year, var, mode = 0):
  ceval = CorrectionSet.from_file("C:/Users/slatt/Downloads/POGJsons/masterJSON.json") #Load JSON file with Trigger Efficiencies

  if lep1pt > 200.0: #No Trigger Scale Factor Applied (Out of bounds Pt)
    return getSFMu(lep1pt, lep1eta, year, var)*getSFMu(lep2pt, lep2eta, year, var)

  if mode == 0: #Pt-Eta Binning for Trigger Scale Factors:
    if var > 0:
      if ceval['UL-PtEta-Trigger-SFs'].evaluate(year,"sf-up","Muon",lep1eta, lep1pt) < 0.25: #Make sure uncertainty in bin is not too high
        return getSFMu(lep1pt, lep1eta, year, var)*getSFMu(lep2pt, lep2eta, year, var)*(ceval['UL-PtEta-Trigger-SFs'].evaluate(year,"sf","Muon",lep1eta, lep1pt) + var * ceval['UL-PtEta-Trigger-SFs'].evaluate(year,"sf-up","Muon",lep1eta, lep1pt))
    else:
      if ceval['UL-PtEta-Trigger-SFs'].evaluate(year,"sf-down","Muon",lep1eta, lep1pt) < 0.25: #Make sure uncertainty in bin is not too high
        return getSFMu(lep1pt, lep1eta, year, var)*getSFMu(lep2pt, lep2eta, year, var)*(ceval['UL-PtEta-Trigger-SFs'].evaluate(year,"sf","Muon",lep1eta, lep1pt) + var * ceval['UL-PtEta-Trigger-SFs'].evaluate(year,"sf-down","Muon",lep1eta, lep1pt))

  if mode == 1: #Pt-Pt Binning for Trigger Scale Factors:
    if var > 0:
      if ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf-up","Muon",lep2pt, lep1pt) < 0.25: #Make sure uncertainty in bin is not too high
        return getSFMu(lep1pt, lep1eta, year, var)*getSFMu(lep2pt, lep2eta, year, var)*(ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf","Muon",lep2pt, lep1pt) + var * ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf-up","Muon",lep2pt, lep1pt))
    else:
      if ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf-down","Muon",lep2pt, lep1pt) < 0.25: #Make sure uncertainty in bin is not too high
        return getSFMu(lep1pt, lep1eta, year, var)*getSFMu(lep2pt, lep2eta, year, var)*(ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf","Muon",lep2pt, lep1pt) + var * ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf-down","Muon",lep2pt, lep1pt))

  return getSFMu(lep1pt, lep1eta, year, var)*getSFMu(lep2pt, lep2eta, year, var)

def getSF2El(lep1pt, lep1eta, lep2pt, lep2eta, year, var, mode  = 0):
  ceval = CorrectionSet.from_file("C:/Users/slatt/Downloads/POGJsons/masterJSON.json") #Load JSON file with Trigger Efficiencies
  ...
  if mode == 0: #Pt-Eta Binning for Trigger Scale Factors:
    if var > 0:
      if ceval['UL-PtEta-Trigger-SFs'].evaluate(year,"sf-up","Electron",lep1eta, lep1pt) < 0.25: #Make sure uncertainty in bin is not too high
        return getSFEl(lep1pt, lep1eta, year, var)*getSFEl(lep2pt, lep2eta, year, var)*(ceval['UL-PtEta-Trigger-SFs'].evaluate(year,"sf","Electron",lep1eta, lep1pt) + var * ceval['UL-PtEta-Trigger-SFs'].evaluate(year,"sf-up","Electron",lep1eta, lep1pt))
    else:
      if ceval['UL-PtEta-Trigger-SFs'].evaluate(year,"sf-down","Electron",lep1eta, lep1pt) < 0.25: #Make sure uncertainty in bin is not too high
        return getSFEl(lep1pt, lep1eta, year, var)*getSFEl(lep2pt, lep2eta, year, var)*(ceval['UL-PtEta-Trigger-SFs'].evaluate(year,"sf","Electron",lep1eta, lep1pt) + var * ceval['UL-PtEta-Trigger-SFs'].evaluate(year,"sf-down","Electron",lep1eta, lep1pt))

  if mode == 1: #Pt-Pt Binning for Trigger Scale Factors:
    if var > 0:
      if ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf-up","Electron",lep2pt, lep1pt) < 0.25: #Make sure uncertainty in bin is not too high
        return getSFEl(lep1pt, lep1eta, year, var)*getSFEl(lep2pt, lep2eta, year, var)*(ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf","Electron",lep2pt, lep1pt) + var * ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf-up","Electron",lep2pt, lep1pt))
    else:
      if ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf-down","Electron",lep2pt, lep1pt) < 0.25: #Make sure uncertainty in bin is not too high
        return getSFEl(lep1pt, lep1eta, year, var)*getSFEl(lep2pt, lep2eta, year, var)*(ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf","Electron",lep2pt, lep1pt) + var * ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf-down","Electron",lep2pt, lep1pt))

  return getSFEl(lep1pt, lep1eta, year, var)*getSFEl(lep2pt, lep2eta, year, var)

def getSF(nmu, lep1pt, lep1eta, lep2pt, lep2eta, year, var=0, mode = 0):
  if nmu == 2:
...
# Mode = 0 for Pt-Eta Trigger SFs, Mode = 1 for Lead/Sublead Pt Trigger SFs
def SF(x, year, var=0, mode = 0): # Var encodes the uncertainty-varied SF (+-1 for varying muons, +-2 for varying electrons)
  w = np.array([])
