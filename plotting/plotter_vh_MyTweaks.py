  ...
  def initAll(self, options):
    ...
    for f in self.config["files"]:
      ...
      try:
        ...
        if options.toLoad:
          a = pd.HDFStore(f, "r")
          meow = a.get_storer("SR").attrs.metadata["gensumweight"]
          a.close()
    ...
    for s in self.samples:
      ...
      if chunks == 1:
        ...
        for i in range(chunks-1):
          ...
          jobfile.write("cd /eos/user/g/gdecastr/SUEPCoffea_dask/plotting/\n")
...
if __name__ == "__main__":
  ...
  if options.queue:
    ...
    os.system("rm submit_3.sub")
    subfile = open("submit_3.sub", "w")
    ...
    subfile.close()
    if not(options.pretend): os.system("condor_submit -spool submit_3.sub")
