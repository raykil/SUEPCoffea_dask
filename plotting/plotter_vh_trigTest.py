   ...
   if True:
    ...
    for s in self.samples:
      ...
      if s.isBackground():
        ...
        if not(back): 
          back = s.histos[pname]["total"].Clone("total_background")
        else: back.Add(s.histos[pname]["total"])
    ...
    tlr.AddEntry(denbar, "Signal syst.+stat. Unc." if self.doSyst else options.ratiostatlabel, "f")
    if self.doSyst:
