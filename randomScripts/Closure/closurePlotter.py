...
def checkClosure(fullhisto, dicvar1, dicvar2, sigs, customBins):
...
while first_bin < second_bin:
    first_bin += 5
    customBins.append(first_bin)
customBins.append(100)

for var1 in ["N_C"]:
    for var2 in ["jet1pt"]:
        if os.path.isfile(inp + "/2D_%s_%s.root" % (var1, var2)):
            tf = ROOT.TFile(inp + "/2D_%s_%s.root" % (var1, var2), "READ")
            th = tf.Get(sys.argv[2] + ";1" if "total" in sys.argv[2] else "2D_%s_%s_" % (var1, var2) + sys.argv[2])
            tl = tf.GetListOfKeys()
            sigs = []
            checkClosure(th, var[var1], var[var2], sigs, customBins)
