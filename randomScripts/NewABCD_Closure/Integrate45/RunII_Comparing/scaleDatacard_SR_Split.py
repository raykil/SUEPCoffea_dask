...
def modify_script(input_file, output_file, closureShapes_f, closureShapeUncs_f, Normalization_f, NormalizationUncs_f, year):
    ...
    pattern_BackNorm = r"rSRn11(?:16APV|16|17|18)\s*rateParam"
    ...
    with open(input_file, "r") as input_file, open(output_file, "w") as output_file:
        for line in input_file:
            ...
            if line.startswith(("r2018SR_SRbin",
                                "r2017SR_SRbin",
                                "r2016SR_SRbin",
                                "r2016APVSR_SRbin",
                                "rRunIISR_SRbin")):
                ...
                output_file.write(line+ "\n")
        print()
        output_file.write(closureSyst(closureSkeleton, 'Normalization'+year, 'lnN', [Normalization['r'+year+'SR'][0] for element in closureShapes['r'+year+'SR']]))
        output_file.write('\n')
        output_file.write(closureSyst(closureSkeleton, 'closure'+year, 'lnN', [element for element in closureShapes['r'+year+'SR']]))
        output_file.write('\n')
        for i in range(0,6):
            output_file.write(closureUncSyst(closureSkeleton, 'closure'+year+'_bin'+str(i+1), 'lnN', closureShapeUncs['r'+year+'SR'][i] + 1, i))
            output_file.write('\n')
            output_file.write(closureUncSyst(closureSkeleton, 'Normalization'+year+'_bin'+str(i+1), 'lnN', NormalizationUncs['r'+year+'SR'][1] + 1, i))
            output_file.write('\n')
        #output_file.write(closureSyst(closureSkeleton, 'Normalization'+year, 'lnN', [NormalizationUncs['r'+year+'SR'][1]+1 for _ in range (8)]))
        #output_file.write('\n')
...
if __name__ == "__main__":
    ...
    filtered_files = [file for file in all_files if file.startswith("CombinedSR_SUEP_hadronic_mS125_mD") and file.endswith(".txt") and "CommonBounds" not in file and "FixedNorm" not in file and "Split" not in file]
