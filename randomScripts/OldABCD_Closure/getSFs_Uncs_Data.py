...
def calculate_ratios(input_file_path):
    ...
    bin_edges = [20, 25, 30, 35, 40, 45, 50, 55]
...
def process_subdirectory(directory):
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.endswith("total_data.root"):  # Ensure it only processes .root files
...
if __name__ == "__main__":
    root_directory = "/eos/user/g/gdecastr/SUEPCoffea_dask/NominalClosure_Corrected/Comparing"

    output_file_path = "/eos/user/g/gdecastr/SUEPCoffea_dask/NominalClosure_Corrected/Comparing/ClosureSFs_DataUncs.txt"
