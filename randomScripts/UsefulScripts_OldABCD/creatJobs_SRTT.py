...
iJ = 0
allFiles = os.listdir("RunII_CRTT")

for f in os.listdir("RunII_CRTT"):
  if not("CombinedSRTT_" in f): continue
  if not("txt" in f): continue
  if "higgsCombine" in f: continue
  print(f)
  froot = f.replace(".txt","")
  if checkFile("RunII/higgsCombine%s.AsymptoticLimits.mH120.root"%froot) and checkFile("RunII/higgsCombine%s.Significance.mH120.root"%froot) and checkFile("RunII/higgsCombine%s.FitDiagnostics.mH120.root"%froot): continue
  print("Pass")
  iJ += 1
  ...
  fil.write("cmsenv\n")
  fil.write("cd /eos/user/g/gdecastr/SUEPCoffea_dask/Cards/HighStats/RunII_CRTT/\n")
