		...
		for var in self.varsToDo:
			...
			if self.doTracks:
				...
				if self.doClusters:
					...
					cutZpt = (self.Zcands.pt > 10) #Looser to get different Higgs boosts
		...
		if self.doTracks:
			...
			if self.isSpherable:
				if self.doClusters and self.isClusterable:
					...
					out["band_pt"]      = self.optimalBand.pt[:]
					out["band_eta"]     = self.optimalBand.eta[:]
					out["band_phi"]     = self.optimalBand.phi[:]
					#out["band_ntracks"] = ak.num(self.tracksinOptimalBand[:], axis = -1)
