...
class SUEP_cluster(processor.ProcessorABC):
    ...
    def clusterizeTracks_Cambridge_1p5(self, events, tracks):
        # anti-kt, dR=1.5 jets
        ...
        if not smallEvents:
            jetdef = fastjet.JetDefinition(fastjet.cambridge_algorithm, 1.5)
    ...
    def clusterizeTracks_Cambridge_1p2(self, events, tracks):
        # anti-kt, dR=1.5 jets
        smallEvents = len(events) < 5000 # This is a trick so the job reserves enough memory for the clustering step and FastJet doesn't go into overflow
        if not smallEvents:
            jetdef = fastjet.JetDefinition(fastjet.cambridge_algorithm, 1.2)
            cluster = fastjet.ClusterSequence(tracks, jetdef)
            ak15_jets	= ak.with_name(cluster.inclusive_jets(min_pt=0),"Momentum4D")	# These are the ak15_jets
            ak15_consts = ak.with_name(cluster.constituents(min_pt=0),"Momentum4D")		# And these are the collections of constituents of the ak15_jets
        else: #With few events/file the thing crashes because of FastJet so we are going to create "fake" events
    ...
    def clusterizeTracks_Cambridge_1p0(self, events, tracks):
        # anti-kt, dR=1.5 jets
        smallEvents = len(events) < 5000 # This is a trick so the job reserves enough memory for the clustering step and FastJet doesn't go into overflow
        if not smallEvents:
            jetdef = fastjet.JetDefinition(fastjet.cambridge_algorithm, 1.0)
            cluster = fastjet.ClusterSequence(tracks, jetdef)
            ak15_jets	= ak.with_name(cluster.inclusive_jets(min_pt=0),"Momentum4D")	# These are the ak15_jets
            ak15_consts = ak.with_name(cluster.constituents(min_pt=0),"Momentum4D")		# And these are the collections of constituents of the ak15_jets
        else: #With few events/file the thing crashes because of FastJet so we are going to create "fake" events
    ...
    def clusterizeTracks_Cambridge_p8(self, events, tracks):
        # anti-kt, dR=1.5 jets
        smallEvents = len(events) < 5000 # This is a trick so the job reserves enough memory for the clustering step and FastJet doesn't go into overflow
        if not smallEvents:
    ...
    def clusterizeTracks_Cambridge_2p0(self, events, tracks):
        # anti-kt, dR=1.5 jets
        smallEvents = len(events) < 5000 # This is a trick so the job reserves enough memory for the clustering step and FastJet doesn't go into overflow
        if not smallEvents:
            jetdef = fastjet.JetDefinition(fastjet.cambridge_algorithm, 2.0)
            cluster = fastjet.ClusterSequence(tracks, jetdef)
            ak15_jets	= ak.with_name(cluster.inclusive_jets(min_pt=0),"Momentum4D")	# These are the ak15_jets
            ak15_consts = ak.with_name(cluster.constituents(min_pt=0),"Momentum4D")		# And these are the collections of constituents of the ak15_jets
        else: #With few events/file the thing crashes because of FastJet so we are going to create "fake" events
    ...
    def clusterizeTracks_kT_1p5(self, events, tracks):
        # anti-kt, dR=1.5 jets
        smallEvents = len(events) < 5000 # This is a trick so the job reserves enough memory for the clustering step and FastJet doesn't go into overflow
        if not smallEvents:
    ...
    def clusterizeTracks_kT_1p2(self, events, tracks):
        # anti-kt, dR=1.5 jets
        smallEvents = len(events) < 5000 # This is a trick so the job reserves enough memory for the clustering step and FastJet doesn't go into overflow
        if not smallEvents:
            jetdef = fastjet.JetDefinition(fastjet.kt_algorithm, 1.2)
            cluster = fastjet.ClusterSequence(tracks, jetdef)
            ak15_jets	= ak.with_name(cluster.inclusive_jets(min_pt=0),"Momentum4D")	# These are the ak15_jets
            ak15_consts = ak.with_name(cluster.constituents(min_pt=0),"Momentum4D")		# And these are the collections of constituents of the ak15_jets
        else: #With few events/file the thing crashes because of FastJet so we are going to create "fake" events
    ...
    def clusterizeTracks_kT_1p0(self, events, tracks):
        # anti-kt, dR=1.5 jets
        smallEvents = len(events) < 5000 # This is a trick so the job reserves enough memory for the clustering step and FastJet doesn't go into overflow
        if not smallEvents:
    ...
    def clusterizeTracks_kT_p8(self, events, tracks):
        # anti-kt, dR=1.5 jets
        smallEvents = len(events) < 5000 # This is a trick so the job reserves enough memory for the clustering step and FastJet doesn't go into overflow
        if not smallEvents:
    ...
    def clusterizeTracks_kT_2p0(self, events, tracks):
        # anti-kt, dR=1.5 jets
        smallEvents = len(events) < 5000 # This is a trick so the job reserves enough memory for the clustering step and FastJet doesn't go into overflow
        if not smallEvents:
            jetdef = fastjet.JetDefinition(fastjet.kt_algorithm, 2.0)
            cluster = fastjet.ClusterSequence(tracks, jetdef)
            ak15_jets	= ak.with_name(cluster.inclusive_jets(min_pt=0),"Momentum4D")	# These are the ak15_jets
            ak15_consts = ak.with_name(cluster.constituents(min_pt=0),"Momentum4D")		# And these are the collections of constituents of the ak15_jets
        else: #With few events/file the thing crashes because of FastJet so we are going to create "fake" events
    ...
    def clusterizeTracks_Ak2p0(self, events, tracks):
        # anti-kt, dR=1.5 jets
        smallEvents = len(events) < 5000 # This is a trick so the job reserves enough memory for the clustering step and FastJet doesn't go into overflow
        if not smallEvents:
            jetdef = fastjet.JetDefinition(fastjet.antikt_algorithm, 2.0)
            cluster = fastjet.ClusterSequence(tracks, jetdef)
            ak15_jets	= ak.with_name(cluster.inclusive_jets(min_pt=0),"Momentum4D")	# These are the ak15_jets
            ak15_consts = ak.with_name(cluster.constituents(min_pt=0),"Momentum4D")		# And these are the collections of constituents of the ak15_jets
        else: #With few events/file the thing crashes because of FastJet so we are going to create "fake" events
    ...
    def clusterizeTracks_Ak1p2(self, events, tracks):
        # anti-kt, dR=1.5 jets
        smallEvents = len(events) < 5000 # This is a trick so the job reserves enough memory for the clustering step and FastJet doesn't go into overflow
        if not smallEvents:
            jetdef = fastjet.JetDefinition(fastjet.antikt_algorithm, 1.2)
            cluster = fastjet.ClusterSequence(tracks, jetdef)
            ak15_jets	= ak.with_name(cluster.inclusive_jets(min_pt=0),"Momentum4D")	# These are the ak15_jets
            ak15_consts = ak.with_name(cluster.constituents(min_pt=0),"Momentum4D")		# And these are the collections of constituents of the ak15_jets
        else: #With few events/file the thing crashes because of FastJet so we are going to create "fake" events
    ...
    def clusterizeTracks_Ak1(self, events, tracks):
        # anti-kt, dR=1.5 jets
        smallEvents = len(events) < 5000 # This is a trick so the job reserves enough memory for the clustering step and FastJet doesn't go into overflow
        if not smallEvents:
    ...
    def clusterizeTracks_Akp8(self, events, tracks):
        # anti-kt, dR=1.5 jets
        smallEvents = len(events) < 5000 # This is a trick so the job reserves enough memory for the clustering step and FastJet doesn't go into overflow
        if not smallEvents:
    ...
    def striptizeTracks(self, tracks, etaWidth):
        ...
        for etaC in etaCenters:
			...
			#print(ak.sum(optimalTracks.fromSUEP, axis = -1))
			#print(ak.num(tracksinBand, axis = -1)) Debugging stuff
			#print(ak.num(optimalTracks, axis = -1))

        optimalBand = ak.zip({
			"px": ak.sum(optimalTracks.px, axis=1),
			"py": ak.sum(optimalTracks.py, axis=1),
			"pz": ak.sum(optimalTracks.pz, axis=1),
			"mass": ak.sum(optimalTracks.mass, axis=1),
			"etaCenter": optimalEta,
		}, with_name="Momentum4D")

        return optimalBand, optimalTracks
    ...
    def sortCluster(self, clust, const):
        highpt_clusters = ak.argsort(clust.pt, axis=1, ascending=False, stable=True)
        clust  = clust[highpt_clusters]
        fromSUEP_temp = const.fromSUEP
        const = const[highpt_clusters]
        const.fromSUEP = fromSUEP_temp[highpt_clusters]
        return clust, const
	...
	# Sorting jets by pt.
        ...
        if self.doTracks:
            ...
            if self.doClusters:
                self.optimalBand, self.tracksinOptimalBand = self.striptizeTracks(self.tracks, 2)[:2]
                self.optimalBand_1p5, self.tracksinOptimalBand_1p5 = self.striptizeTracks(self.tracks, 1.5)[:2]
                self.optimalBand_1p0, self.tracksinOptimalBand_1p0 = self.striptizeTracks(self.tracks, 1.0)[:2]
                self.optimalBand_p75, self.tracksinOptimalBand_p75 = self.striptizeTracks(self.tracks, .75)[:2]
                self.optimalBand_p5, self.tracksinOptimalBand_p5 = self.striptizeTracks(self.tracks, .5)[:2]

                self.events, self.clusters, self.constituents  = self.clusterizeTracks(self.events, self.tracks)[:3]
                self.clusters_1p2, self.constituents_1p2  = self.clusterizeTracks_Ak1p2(self.events, self.tracks)[1:3]
                self.clusters_1p0, self.constituents_1p0  = self.clusterizeTracks_Ak1(self.events, self.tracks)[1:3]
                self.clusters_p8, self.constituents_p8  = self.clusterizeTracks_Akp8(self.events, self.tracks)[1:3]
                self.clusters_2p0, self.constituents_2p0  = self.clusterizeTracks_Ak2p0(self.events, self.tracks)[1:3]

                self.clusters_kT_1p5, self.constituents_kT_1p5  = self.clusterizeTracks_kT_1p5(self.events, self.tracks)[1:3]
                self.clusters_kT_1p2, self.constituents_kT_1p2  = self.clusterizeTracks_kT_1p2(self.events, self.tracks)[1:3]
                self.clusters_kT_1p0, self.constituents_kT_1p0  = self.clusterizeTracks_kT_1p0(self.events, self.tracks)[1:3]
                self.clusters_kT_p8, self.constituents_kT_p8  = self.clusterizeTracks_kT_p8(self.events, self.tracks)[1:3]
                self.clusters_kT_2p0, self.constituents_kT_2p0  = self.clusterizeTracks_kT_2p0(self.events, self.tracks)[1:3]

                self.clusters_cambridge_1p5, self.constituents_cambridge_1p5  = self.clusterizeTracks_Cambridge_1p5(self.events, self.tracks)[1:3]
                self.clusters_cambridge_1p2, self.constituents_cambridge_1p2  = self.clusterizeTracks_Cambridge_1p2(self.events, self.tracks)[1:3]
                self.clusters_cambridge_1p0, self.constituents_cambridge_1p0  = self.clusterizeTracks_Cambridge_1p0(self.events, self.tracks)[1:3]
                self.clusters_cambridge_p8, self.constituents_cambridge_p8  = self.clusterizeTracks_Cambridge_p8(self.events, self.tracks)[1:3]
                self.clusters_cambridge_2p0, self.constituents_cambridge_2p0  = self.clusterizeTracks_Cambridge_2p0(self.events, self.tracks)[1:3]

                self.clusters, self.constituents = self.sortCluster(self.clusters, self.constituents)
                self.clusters_1p2, self.constituents_1p2 = self.sortCluster(self.clusters_1p2, self.constituents_1p2)
                self.clusters_1p0, self.constituents_1p0 = self.sortCluster(self.clusters_1p0, self.constituents_1p0)
                self.clusters_p8, self.constituents_p8 = self.sortCluster(self.clusters_p8, self.constituents_p8)
                self.clusters_2p0, self.constituents_2p0 = self.sortCluster(self.clusters_2p0, self.constituents_2p0)

                self.clusters_kT_1p5, self.constituents_kT_1p5 = self.sortCluster(self.clusters_kT_1p5, self.constituents_kT_1p5)
                self.clusters_kT_1p2, self.constituents_kT_1p2 = self.sortCluster(self.clusters_kT_1p2, self.constituents_kT_1p2)
                self.clusters_kT_1p0, self.constituents_kT_1p0 = self.sortCluster(self.clusters_kT_1p0, self.constituents_kT_1p0)
                self.clusters_kT_p8, self.constituents_kT_p8 = self.sortCluster(self.clusters_kT_p8, self.constituents_kT_p8)
                self.clusters_kT_2p0, self.constituents_kT_2p0 = self.sortCluster(self.clusters_kT_2p0, self.constituents_kT_2p0)

                self.clusters_cambridge_1p5, self.constituents_cambridge_1p5 = self.sortCluster(self.clusters_cambridge_1p5, self.constituents_cambridge_1p5)
                self.clusters_cambridge_1p2, self.constituents_cambridge_1p2 = self.sortCluster(self.clusters_cambridge_1p2, self.constituents_cambridge_1p2)
                self.clusters_cambridge_1p0, self.constituents_cambridge_1p0 = self.sortCluster(self.clusters_cambridge_1p0, self.constituents_cambridge_1p0)
                self.clusters_cambridge_p8, self.constituents_cambridge_p8 = self.sortCluster(self.clusters_cambridge_p8, self.constituents_cambridge_p8)
                self.clusters_cambridge_2p0, self.constituents_cambridge_2p0 = self.sortCluster(self.clusters_cambridge_2p0, self.constituents_cambridge_2p0)
        ...
        for var in self.varsToDo:
            ...
            #outputs["twoleptons"+var] = [self.doAllPlots("twoleptons"+var, csvTag, debug), self.events]
            if not(self.shouldContinueAfterCut(self.events, outputs)): return accumulator
            ...
            if self.doTracks:
                ...
                if self.doClusters:
                    ...
                    if self.doClusters and self.var != "": # As tracks might have changed, need to redo this step
                      ...
                      self.clusters_cambridge_p8, self.constituents_cambridge_p8 = self.sortCluster(self.clusters_cambridge_p8, self.constituents_cambridge_p8)
                      self.clusters_cambridge_2p0, self.constituents_cambridge_2p0 = self.sortCluster(self.clusters_cambridge_2p0, self.constituents_cambridge_2p0)
                    #outputs["onecluster"+var] = [self.doAllPlots("onecluster"+var, csvTag, debug), self.events]
                    if not(self.shouldContinueAfterCut(self.events, outputs)): return accumulator
                    ...
                    outputs["SR"+var] = [self.doAllPlots("SR"+var, csvTag, debug), self.events]
    ...
    def applyCutToAllCollections(self, cut): # Cut has to by a selection applicable across all collections, i.e. something defined per event
        ...
        if self.doTracks:
            ...
            if self.doClusters:
                ...
                fromSUEP_temp = self.tracksinOptimalBand.fromSUEP
                self.tracksinOptimalBand = self.tracksinOptimalBand[cut]
                self.tracksinOptimalBand.fromSUEP = fromSUEP_temp[cut]

                self.optimalBand = self.optimalBand[cut]

                fromSUEP_temp = self.tracksinOptimalBand_1p5.fromSUEP
                self.tracksinOptimalBand_1p5 = self.tracksinOptimalBand_1p5[cut]
                self.tracksinOptimalBand_1p5.fromSUEP = fromSUEP_temp[cut]

                self.optimalBand_1p5 = self.optimalBand_1p5[cut]

                fromSUEP_temp = self.tracksinOptimalBand_1p0.fromSUEP
                self.tracksinOptimalBand_1p0 = self.tracksinOptimalBand_1p0[cut]
                self.tracksinOptimalBand_1p0.fromSUEP = fromSUEP_temp[cut]

                self.optimalBand_1p0 = self.optimalBand_1p0[cut]

                fromSUEP_temp = self.tracksinOptimalBand_p75.fromSUEP
                self.tracksinOptimalBand_p75 = self.tracksinOptimalBand_p75[cut]
                self.tracksinOptimalBand_p75.fromSUEP = fromSUEP_temp[cut]

                self.optimalBand_p75 = self.optimalBand_p75[cut]

                fromSUEP_temp = self.tracksinOptimalBand_p5.fromSUEP
                self.tracksinOptimalBand_p5 = self.tracksinOptimalBand_p5[cut]
                self.tracksinOptimalBand_p5.fromSUEP = fromSUEP_temp[cut]

                self.optimalBand_p5 = self.optimalBand_p5[cut]
                ...
                self.clusters_2p0     = self.clusters_2p0[cut]
                fromSUEP_temp = self.constituents_2p0.fromSUEP
                self.constituents_2p0 = self.constituents_2p0[cut]
                self.constituents_2p0.fromSUEP = fromSUEP_temp[cut]

                self.clusters_1p2     = self.clusters_1p2[cut]
                fromSUEP_temp = self.constituents_1p2.fromSUEP
                self.constituents_1p2 = self.constituents_1p2[cut]
                self.constituents_1p2.fromSUEP = fromSUEP_temp[cut]

                self.clusters_1p0     = self.clusters_1p0[cut]
                fromSUEP_temp = self.constituents_1p0.fromSUEP
                self.constituents_1p0 = self.constituents_1p0[cut]
                self.constituents_1p0.fromSUEP = fromSUEP_temp[cut]

                self.clusters_p8    = self.clusters_p8[cut]
                fromSUEP_temp = self.constituents_p8.fromSUEP
                self.constituents_p8 = self.constituents_p8[cut]
                self.constituents_p8.fromSUEP = fromSUEP_temp[cut]

                self.clusters_kT_2p0 = self.clusters_kT_2p0[cut]
                fromSUEP_temp = self.constituents_kT_2p0.fromSUEP
                self.constituents_kT_2p0 = self.constituents_kT_2p0[cut]
                self.constituents_kT_2p0.fromSUEP = fromSUEP_temp[cut]

                self.clusters_kT_1p5 = self.clusters_kT_1p5[cut]
                fromSUEP_temp = self.constituents_kT_1p5.fromSUEP
                self.constituents_kT_1p5 = self.constituents_kT_1p5[cut]
                self.constituents_kT_1p5.fromSUEP = fromSUEP_temp[cut]

                self.clusters_kT_1p2 = self.clusters_kT_1p2[cut]
                fromSUEP_temp = self.constituents_kT_1p2.fromSUEP
                self.constituents_kT_1p2 = self.constituents_kT_1p2[cut]
                self.constituents_kT_1p2.fromSUEP = fromSUEP_temp[cut]

                self.clusters_kT_1p0 = self.clusters_kT_1p0[cut]
                fromSUEP_temp = self.constituents_kT_1p0.fromSUEP
                self.constituents_kT_1p0 = self.constituents_kT_1p0[cut]
                self.constituents_kT_1p0.fromSUEP = fromSUEP_temp[cut]

                self.clusters_kT_p8 = self.clusters_kT_p8[cut]
                fromSUEP_temp = self.constituents_kT_p8.fromSUEP
                self.constituents_kT_p8= self.constituents_kT_p8[cut]
                self.constituents_kT_p8.fromSUEP = fromSUEP_temp[cut]

                self.clusters_cambridge_2p0 = self.clusters_cambridge_2p0[cut]
                fromSUEP_temp = self.constituents_cambridge_2p0.fromSUEP
                self.constituents_cambridge_2p0 = self.constituents_cambridge_2p0[cut]
                self.constituents_cambridge_2p0.fromSUEP = fromSUEP_temp[cut]

                self.clusters_cambridge_1p5 = self.clusters_cambridge_1p5[cut]
                fromSUEP_temp = self.constituents_cambridge_1p5.fromSUEP
                self.constituents_cambridge_1p5 = self.constituents_cambridge_1p5[cut]
                self.constituents_cambridge_1p5.fromSUEP = fromSUEP_temp[cut]

                self.clusters_cambridge_1p2 = self.clusters_cambridge_1p2[cut]
                fromSUEP_temp = self.constituents_cambridge_1p2.fromSUEP
                self.constituents_cambridge_1p2 = self.constituents_cambridge_1p2[cut]
                self.constituents_cambridge_1p2.fromSUEP = fromSUEP_temp[cut]

                self.clusters_cambridge_1p0 = self.clusters_cambridge_1p0[cut]
                fromSUEP_temp = self.constituents_cambridge_1p0.fromSUEP
                self.constituents_cambridge_1p0 = self.constituents_cambridge_1p0[cut]
                self.constituents_cambridge_1p0.fromSUEP = fromSUEP_temp[cut]

                self.clusters_cambridge_p8 = self.clusters_cambridge_p8[cut]
                fromSUEP_temp = self.constituents_cambridge_p8.fromSUEP
                self.constituents_cambridge_p8= self.constituents_cambridge_p8[cut]
                self.constituents_cambridge_p8.fromSUEP = fromSUEP_temp[cut]
        ...
        if self.doTracks:
            ...
            if self.isSpherable:
                if self.doClusters and self.isClusterable:
                    ...
                    df = pd.DataFrame({"Ak15DeltaR2" : self.genH[:,1].deltaR2(self.clusters[:,0]),\
                                        "Ak12DeltaR2" : self.genH[:,1].deltaR2(self.clusters_1p2[:,0]),\
                                        "Ak1DeltaR2" : self.genH[:,1].deltaR2(self.clusters_1p0[:,0]),\
                                        "Ak8DeltaR2" : self.genH[:,1].deltaR2(self.clusters_p8[:,0]),\
                                        "Ak20DeltaR2" : self.genH[:,1].deltaR2(self.clusters_2p0[:,0]),\
                                        "Ak15BetaRes" : (self.clusters[:,0].beta-self.genH.beta[:,1])/self.genH.beta[:,1],\
                                        "Ak12BetaRes" : (self.clusters_1p2[:,0].beta-self.genH.beta[:,1])/self.genH.beta[:,1],\
                                        "Ak10BetaRes" : (self.clusters_1p0[:,0].beta-self.genH.beta[:,1])/self.genH.beta[:,1],\
                                        "Ak8BetaRes" : (self.clusters_p8[:,0].beta-self.genH.beta[:,1])/self.genH.beta[:,1],\
                                        "Ak20BetaRes" : (self.clusters_2p0[:,0].beta-self.genH.beta[:,1])/self.genH.beta[:,1],\
                                        "LeadingAk15Purity" : ak.sum(self.constituents.fromSUEP[:,0], axis = -1)/ak.num(self.constituents[:,0]),\
                                        "LeadingAk12Purity" : ak.sum(self.constituents_1p2.fromSUEP[:,0], axis = -1)/ak.num(self.constituents_1p2[:,0]),\
                                        "LeadingAk10Purity" : ak.sum(self.constituents_1p0.fromSUEP[:,0], axis = -1)/ak.num(self.constituents_1p0[:,0]),\
                                        "LeadingAk8Purity" : ak.sum(self.constituents_p8.fromSUEP[:,0], axis = -1)/ak.num(self.constituents_p8[:,0]),\
                                        "LeadingAk20Purity" : ak.sum(self.constituents_2p0.fromSUEP[:,0], axis = -1)/ak.num(self.constituents_2p0[:,0]),\
                                        "LeadingAk15Eff" : ak.sum(self.constituents.fromSUEP[:,0], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "LeadingAk12Eff" : ak.sum(self.constituents_1p2.fromSUEP[:,0], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "LeadingAk10Eff" : ak.sum(self.constituents_1p0.fromSUEP[:,0], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "LeadingAk8Eff" : ak.sum(self.constituents_p8.fromSUEP[:,0], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "LeadingAk20Eff" : ak.sum(self.constituents_2p0.fromSUEP[:,0], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        })
                    df.to_csv(self.output_location+"/"+csvTag+'_Ak.csv', index=False)
                    df = pd.DataFrame({"kT15DeltaR2" : self.genH[:,1].deltaR2(self.clusters_kT_1p5[:,0]),\
                                        "kT12DeltaR2" : self.genH[:,1].deltaR2(self.clusters_kT_1p2[:,0]),\
                                        "kT1DeltaR2" : self.genH[:,1].deltaR2(self.clusters_kT_1p0[:,0]),\
                                        "kT8DeltaR2" : self.genH[:,1].deltaR2(self.clusters_kT_p8[:,0]),\
                                        "kT20DeltaR2" : self.genH[:,1].deltaR2(self.clusters_kT_2p0[:,0]),\
                                        "kT15BetaRes" : (self.clusters_kT_1p5[:,0].beta-self.genH.beta[:,1])/self.genH.beta[:,1],\
                                        "kT12BetaRes" : (self.clusters_kT_1p2[:,0].beta-self.genH.beta[:,1])/self.genH.beta[:,1],\
                                        "kT10BetaRes" : (self.clusters_kT_1p0[:,0].beta-self.genH.beta[:,1])/self.genH.beta[:,1],\
                                        "kT8BetaRes" : (self.clusters_kT_p8[:,0].beta-self.genH.beta[:,1])/self.genH.beta[:,1],\
                                        "kT20BetaRes" : (self.clusters_kT_2p0[:,0].beta-self.genH.beta[:,1])/self.genH.beta[:,1],\
                                        "LeadingkT15Purity" : ak.sum(self.constituents_kT_1p5.fromSUEP[:,0], axis = -1)/ak.num(self.constituents_kT_1p5[:,0]),\
                                        "LeadingkT12Purity" : ak.sum(self.constituents_kT_1p2.fromSUEP[:,0], axis = -1)/ak.num(self.constituents_kT_1p2[:,0]),\
                                        "LeadingkT10Purity" : ak.sum(self.constituents_kT_1p0.fromSUEP[:,0], axis = -1)/ak.num(self.constituents_kT_1p0[:,0]),\
                                        "LeadingkT8Purity" : ak.sum(self.constituents_kT_p8.fromSUEP[:,0], axis = -1)/ak.num(self.constituents_kT_p8[:,0]),\
                                        "LeadingkT20Purity" : ak.sum(self.constituents_kT_2p0.fromSUEP[:,0], axis = -1)/ak.num(self.constituents_kT_2p0[:,0]),\
                                        "LeadingkT15Eff" : ak.sum(self.constituents_kT_1p5.fromSUEP[:,0], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "LeadingkT12Eff" : ak.sum(self.constituents_kT_1p2.fromSUEP[:,0], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "LeadingkT10Eff" : ak.sum(self.constituents_kT_1p0.fromSUEP[:,0], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "LeadingkT8Eff" : ak.sum(self.constituents_kT_p8.fromSUEP[:,0], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "LeadingkT20Eff" : ak.sum(self.constituents_kT_2p0.fromSUEP[:,0], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        })
                    df.to_csv(self.output_location+"/"+csvTag+'_kT.csv', index=False)
                    df = pd.DataFrame({"cambridge15DeltaR2" : self.genH[:,1].deltaR2(self.clusters_cambridge_1p5[:,0]),\
                                        "cambridge12DeltaR2" : self.genH[:,1].deltaR2(self.clusters_cambridge_1p2[:,0]),\
                                        "cambridge1DeltaR2" : self.genH[:,1].deltaR2(self.clusters_cambridge_1p0[:,0]),\
                                        "cambridge8DeltaR2" : self.genH[:,1].deltaR2(self.clusters_cambridge_p8[:,0]),\
                                        "cambridge20DeltaR2" : self.genH[:,1].deltaR2(self.clusters_cambridge_2p0[:,0]),\
                                        "cambridge15BetaRes" : (self.clusters_cambridge_1p5[:,0].beta-self.genH.beta[:,1])/self.genH.beta[:,1],\
                                        "cambridge12BetaRes" : (self.clusters_cambridge_1p2[:,0].beta-self.genH.beta[:,1])/self.genH.beta[:,1],\
                                        "cambridge10BetaRes" : (self.clusters_cambridge_1p0[:,0].beta-self.genH.beta[:,1])/self.genH.beta[:,1],\
                                        "cambridge8BetaRes" : (self.clusters_cambridge_p8[:,0].beta-self.genH.beta[:,1])/self.genH.beta[:,1],\
                                        "cambridge20BetaRes" : (self.clusters_cambridge_2p0[:,0].beta-self.genH.beta[:,1])/self.genH.beta[:,1],\
                                        "Leadingcambridge15Purity" : ak.sum(self.constituents_cambridge_1p5.fromSUEP[:,0], axis = -1)/ak.num(self.constituents_cambridge_1p5[:,0]),\
                                        "Leadingcambridge12Purity" : ak.sum(self.constituents_cambridge_1p2.fromSUEP[:,0], axis = -1)/ak.num(self.constituents_cambridge_1p2[:,0]),\
                                        "Leadingcambridge10Purity" : ak.sum(self.constituents_cambridge_1p0.fromSUEP[:,0], axis = -1)/ak.num(self.constituents_cambridge_1p0[:,0]),\
                                        "Leadingcambridge8Purity" : ak.sum(self.constituents_cambridge_p8.fromSUEP[:,0], axis = -1)/ak.num(self.constituents_cambridge_p8[:,0]),\
                                        "Leadingcambridge20Purity" : ak.sum(self.constituents_cambridge_2p0.fromSUEP[:,0], axis = -1)/ak.num(self.constituents_cambridge_2p0[:,0]),\
                                        "Leadingcambridge15Eff" : ak.sum(self.constituents_cambridge_1p5.fromSUEP[:,0], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "Leadingcambridge12Eff" : ak.sum(self.constituents_cambridge_1p2.fromSUEP[:,0], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "Leadingcambridge10Eff" : ak.sum(self.constituents_cambridge_1p0.fromSUEP[:,0], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "Leadingcambridge8Eff" : ak.sum(self.constituents_cambridge_p8.fromSUEP[:,0], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "Leadingcambridge20Eff" : ak.sum(self.constituents_cambridge_2p0.fromSUEP[:,0], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        })
                    df.to_csv(self.output_location+"/"+csvTag+'_Cambridge.csv', index=False)
                    df = pd.DataFrame({"Strip20DeltaR2" : np.array([self.genH[i][1].deltaR2(self.optimalBand[i]) for i in range(len(self.optimalBand))]),\
                                       "Strip20BetaRes" : np.array([(self.optimalBand[i].beta - self.genH.beta[i][1])/self.genH.beta[i][1] for i in range(len(self.optimalBand))]),\
                                        "Strip20Purity" : ak.sum(self.tracksinOptimalBand.fromSUEP[:], axis = -1)/ak.num(self.tracksinOptimalBand[:]),\
                                        "Strip20Eff" : ak.sum(self.tracksinOptimalBand.fromSUEP[:], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "Strip20Center" : np.array([i for i in self.optimalBand.etaCenter]),\
                                        "Strip15DeltaR2" : np.array([self.genH[i][1].deltaR2(self.optimalBand_1p5[i]) for i in range(len(self.optimalBand_1p5))]),\
                                       "Strip15BetaRes" : np.array([(self.optimalBand_1p5[i].beta - self.genH.beta[i][1])/self.genH.beta[i][1] for i in range(len(self.optimalBand_1p5))]),\
                                        "Strip15Purity" : ak.sum(self.tracksinOptimalBand_1p5.fromSUEP[:], axis = -1)/ak.num(self.tracksinOptimalBand_1p5[:]),\
                                        "Strip15Eff" : ak.sum(self.tracksinOptimalBand_1p5.fromSUEP[:], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "Strip15Center" : np.array([i for i in self.optimalBand_1p5.etaCenter]),\
                                        "Strip10DeltaR2" : np.array([self.genH[i][1].deltaR2(self.optimalBand_1p0[i]) for i in range(len(self.optimalBand_1p0))]),\
                                       "Strip10BetaRes" : np.array([(self.optimalBand_1p0[i].beta - self.genH.beta[i][1])/self.genH.beta[i][1] for i in range(len(self.optimalBand_1p0))]),\
                                        "Strip10Purity" : ak.sum(self.tracksinOptimalBand_1p0.fromSUEP[:], axis = -1)/ak.num(self.tracksinOptimalBand_1p0[:]),\
                                        "Strip10Eff" : ak.sum(self.tracksinOptimalBand_1p0.fromSUEP[:], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "Strip10Center" : np.array([i for i in self.optimalBand_1p0.etaCenter]),\
                                        "Strip75DeltaR2" : np.array([self.genH[i][1].deltaR2(self.optimalBand_p75[i]) for i in range(len(self.optimalBand_p75))]),\
                                       "Strip75BetaRes" : np.array([(self.optimalBand_p75[i].beta - self.genH.beta[i][1])/self.genH.beta[i][1] for i in range(len(self.optimalBand_p75))]),\
                                        "Strip75Purity" : ak.sum(self.tracksinOptimalBand_p75.fromSUEP[:], axis = -1)/ak.num(self.tracksinOptimalBand_p75[:]),\
                                        "Strip75Eff" : ak.sum(self.tracksinOptimalBand_p75.fromSUEP[:], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "Strip75Center" : np.array([i for i in self.optimalBand_p75.etaCenter]),\
                                        "Strip5DeltaR2" : np.array([self.genH[i][1].deltaR2(self.optimalBand_p5[i]) for i in range(len(self.optimalBand_p5))]),\
                                       "Strip5BetaRes" : np.array([(self.optimalBand_p5[i].beta - self.genH.beta[i][1])/self.genH.beta[i][1] for i in range(len(self.optimalBand_p5))]),\
                                        "Strip5Purity" : ak.sum(self.tracksinOptimalBand_p5.fromSUEP[:], axis = -1)/ak.num(self.tracksinOptimalBand_p5[:]),\
                                        "Strip5Eff" : ak.sum(self.tracksinOptimalBand_p5.fromSUEP[:], axis = -1)/ak.sum(self.tracks.fromSUEP, axis = -1),\
                                        "Strip5Center" : np.array([i for i in self.optimalBand_p5.etaCenter]),\
                                        })
                    df.to_csv(self.output_location+"/"+csvTag+'_Strip.csv', index=False)
                    df = pd.DataFrame({"GenHiggsPx" : self.genH.px[:,1],\
                                        "GenHiggsPy" : self.genH.py[:,1],\
                                        "GenHiggsPz" : self.genH.pz[:,1],\
                                        "GenHiggsM" : self.genH.mass[:,1],\
                                        })
                    df.to_csv(self.output_location+"/"+csvTag+'_GenHiggs.csv', index=False)
                    meow
