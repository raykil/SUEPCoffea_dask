...
class SUEP_cluster(processor.ProcessorABC):
        ...
        for var in self.varsToDo:
            ...
            if self.doTracks:
                ...
                if self.doClusters:
                    ...
                    self.applyCutToAllCollections(ak.num(self.constituents[:,0], axis = 1) >= 50)
                    self.applyCutToAllCollections((self.jets.pt[:,0] >= 100) and (self.jets.pt[:,0] <= 250))
    ...
    def doAllPlots(self, channel, debug=True):
        ...
        out["luminosityBlock"] = self.events.luminosityBlock[:]
        out["events"] = self.events.event[:]
        # Object: leptons
    ...
    def getNPU(self):
        if self.isMC:
            if hasattr(self.events, "Pileup"):
                ...
                ak.ones_like(self.events.genWeight)
        else:
