...
class SUEP_cluster(processor.ProcessorABC):
    ...
    def selectByTracks2(self, events, leptons, extraColls = []):
    ...
    def clusterDiff(self, events, tracks, clusters, constitutents):
        print(tracks[0])
        print(clusters[0])
        print(constitutents[0])
        a = ak.num(self.tracks, axis=1)
        b = ak.sum(ak.num(self.constituents, axis=2), axis = 1)
    ...
    def genVsReco(self, events, genH, clusters):
        genH = genH[:,1]
        px_diff = (clusters.px - genH.px)/(genH.px)
        py_diff = (clusters.py - genH.py)/(genH.py)
        pz_diff = (clusters.pz - genH.pz)/(genH.pz)
        mag_diff = (clusters.mag - genH.mag)/(genH.mag)
        mass_diff = (clusters.mass - genH.mass)/(genH.mass)
        e_diff = (clusters.E - genH.E)/(genH.E)
        #for i in range(len(px_diff)):
        #    if np.abs(px_diff[i]) > 20 or np.abs(py_diff[i]) > 20 or np.abs(pz_diff[i]) > 20 or np.abs(mag_diff[i]) > 20 or np.abs(mass_diff[i]) > 20 or np.abs(e_diff[i]) > 20:
        #        print(px_diff[i], py_diff[i], pz_diff[i], mag_diff[i], mass_diff[i], e_diff[i])
        #        print(clusters.px[i], genH.px[i], clusters.py[i], genH.py[i], clusters.pz[i], genH.pz[i], clusters.mag[i], genH.mag[i], clusters.mass[i], genH.mass[i], clusters.e[i], genH.e[i]) 
        #        print('meow')
        return px_diff, py_diff, pz_diff, mag_diff, mass_diff, e_diff
    ...
    def process(self, events):
        ...
        if self.doTracks:
            ...
            self.events, self.tracks = self.selectByTracks2(self.events, self.leptons)[:2]
            if not(self.shouldContinueAfterCut(self.events, outputs)): return accumulator
            ...
            if self.doClusters:
                ...
                #self.clusterDiff(self.events, self.tracks, self.clusters, self.constituents)
        ...
        if self.doTracks:
                ...
                self.px_diff, self.py_diff, self.pz_diff, self.mag_diff, self.mass_diff, self.e_diff = self.genVsReco(self.events, self.genH, self.clusters[:,0])[:6]
                ...
                if self.doClusters:
                    ...
                    outputs["SR"] = [self.doAllPlots("SR", csvTag, debug), self.events]
        # ------------------------------------------------------------------------------
        # -------------------------------- SAVING --------------------------------------
        # ------------------------------------------------------------------------------
        todel = []
    ...
    def doAllPlots(self, channel, csvTag, debug=True):
        ...
        df = pd.DataFrame({"px_diff" : self.px_diff, "py_diff" : self.py_diff, "pz_diff" : self.pz_diff, "mag_diff" : self.mag_diff, "mass_diff" : self.mass_diff, "e_diff" : self.e_diff})
        df.to_csv(self.output_location+"/"+csvTag+'_GenVsReco.csv', index=False)
        if self.doTracks:
            #out["ntracks"]     = ak.num(self.tracks, axis=1)[:]
            maxntracks         = ak.max(ak.num(self.tracks, axis=1))
            #for prop in ["pt", "eta", "phi", "mass", "d0", "d0Err", "dz", "dzErr", "puppiWeight", "puppiWeightNoLep", "vtxChi2", "fromPV", "pvAssocQuality"]:
            for prop in ["d0", "d0Err", "dz", "dzErr", "fromPV"]:
            ...
            if self.isSpherable:
                if self.doClusters and self.isClusterable:
                    ...
                    #Checking track numbers
                    #print(ak.num(self.clusters, axis=1)[:])
                    #print(ak.num(self.tracks, axis=1)[:])
