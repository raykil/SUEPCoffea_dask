...
class SUEP_cluster(processor.ProcessorABC):
    ...
    def selectByTracks(self, events, leptons, extraColls = []):
        ...
        cutPF = (events.PFCands.trkPt >= 0.5) #& \
        #      (events.PFCands.fromPV > 0) & \    
	...
	### LOST TRACKS ###
        ...
        cutLost = (events.lostTracks.pt >= 0.5) #& \
        #(events.lostTracks.fromPV > 0) & \
        ...
        GenParts  = ak.zip({ 
            ...
            "fromSUEP": -1,
            "preShowerParton": False
        }) 
        
        motherPDG = []
        for i,j in zip(range(len(GenParts)), GenParts):
            motherPDG_temp = ak.where(j.motherId > -1, j.pdgId[j.motherId], -999)
            motherPDG.append(ak.Array(motherPDG_temp))
        GenParts.motherPDG = ak.Array(motherPDG)

        GenParts.preShowerParton = ak.where((np.abs(GenParts.pdgId) == 5) | (np.abs(GenParts.pdgId) == 4) | (np.abs(GenParts.pdgId)== 3) | (np.abs(GenParts.pdgId)== 2) | (np.abs(GenParts.pdgId) == 1) | (np.abs(GenParts.pdgId) == 21), True, GenParts.preShowerParton)
        GenParts.preShowerParton = ak.where((GenParts.status == 1) | (GenParts.status == 2), False, GenParts.preShowerParton)
        GenParts.preShowerParton = ak.where((np.abs(GenParts.motherPDG) == 5) | (np.abs(GenParts.motherPDG) == 4) | (np.abs(GenParts.motherPDG)== 3) | (np.abs(GenParts.motherPDG)== 2) | (np.abs(GenParts.motherPDG) == 1) | (np.abs(GenParts.motherPDG) == 21), False, GenParts.preShowerParton)

        preShowerParton = [ak.Array(GenParts.preShowerParton), ak.Array(GenParts.eta), ak.Array(GenParts.pdgId), ak.Array(GenParts.status), ak.Array(GenParts.motherPDG), ak.Array(GenParts.pt)]
        ...
        SimTracks.fromPU   = ak.where(SimTracks.igen > -3, GenParts.fromSUEP[SimTracks.igen] == 0, False)
        SimTracks.fromUE   = ak.where(SimTracks.igen >= 0, GenParts.fromSUEP[SimTracks.igen] == 3, False)
        SimTracks.fromISR   = ak.where(SimTracks.igen >= 0, GenParts.fromSUEP[SimTracks.igen] == 4, False)
        SimTracks.fromZ   = ak.where(SimTracks.igen >= 0, GenParts.fromSUEP[SimTracks.igen] == 2, False)
        ...
        totalTracks.fromPU = ak.where(ak.min(alldr2, axis=2) < 0.01, SimTracks.fromPU[ak.argmin(alldr2, axis=2)], False)
        totalTracks.fromZ = ak.where(ak.min(alldr2, axis=2) < 0.01, SimTracks.fromZ[ak.argmin(alldr2, axis=2)], False)
        totalTracks.fromISR = ak.where(ak.min(alldr2, axis=2) < 0.01, SimTracks.fromISR[ak.argmin(alldr2, axis=2)], False)
        totalTracks.fromUE = ak.where(ak.min(alldr2, axis=2) < 0.01, SimTracks.fromUE[ak.argmin(alldr2, axis=2)], False)
        suepTracks = totalTracks[totalTracks.fromSUEP == True]
        backTracks = totalTracks[totalTracks.fromSUEP == False]
        return events, totalTracks, suepTracks, backTracks, SimTracks, totalTracks, preShowerParton
    ...
    def clusterizeTracks(self, events, tracks):
        ...
        if not smallEvents:
          ...
          #for i in cluster.constituents(min_pt=0)[:,0]:
          #  print(i)
    ...
    def process(self, events):
        ...
        self.doGen      = True # In case we want info on the gen level, we do need it for the buggy DY samples (to get proper stitching)
        ...
        if self.doTracks:
            ...
            self.events, self.tracks, self.sueptracks, self.backtracks, self.simTracks, self.reco, self.preShowerParton= self.selectByTracks(self.events, self.leptons)[:7] # Again, we need leptons to clean the tracks
            if not(self.shouldContinueAfterCut(self.events, outputs)): return accumulator
        ...
        print([ item for sublist in self.tracks.fromSUEP for item in sublist ].count(1))
        if not(self.shouldContinueAfterCut(self.events, outputs)): return accumulator
        if debug: print("%i events pass twoleptons cuts. Doing more stuff..."%len(self.events))

        if self.doTracks:

                temp_SUEP_flag = self.tracks.fromSUEP
                cutOneTrack = (ak.num(self.tracks) != 0)
                self.applyCutToAllCollections(cutOneTrack)

                temp_SUEP_flag = temp_SUEP_flag[cutOneTrack]

                self.tracks.fromSUEP = temp_SUEP_flag
                ...
                if self.doClusters:
                    ...
                    temp_SUEP_flag = temp_SUEP_flag[cutOneCluster]
                    ...
                    if self.doClusters and self.var != "": # As tracks might have changed, need to redo this step
                      ...
                      self.clusters   = self.clusters[highpt_clusters]
                      self.constituents = self.constituents[highpt_clusters]
                    outputs["onecluster"] = [self.doAllPlots("onecluster", csvTag, debug), self.events]
                    if not(self.shouldContinueAfterCut(self.events, outputs)): return accumulator
                    ...
                    temp_SUEP_flag = temp_SUEP_flag[cutZm]
                    ...
                    temp_SUEP_flag = temp_SUEP_flag[cutZpt]
                    ...
                    temp_SUEP_flag = temp_SUEP_flag[cut0tag]
                    ...
                    temp_SUEP_flag = temp_SUEP_flag[cutclusterpt60]
    ...
    def doAllPlots(self, channel, csvTag, debug=True):
        ...
        df = pd.DataFrame({"Sim_fromSUEP" : np.concatenate(self.simTracks.fromSUEP, axis = 0), "Sim_fromZ" : np.concatenate(self.simTracks.fromZ, axis = 0), "Sim_fromISR" : np.concatenate(self.simTracks.fromISR, axis = 0), "Sim_fromPU" : np.concatenate(self.simTracks.fromPU, axis = 0), "Sim_fromUE" : np.concatenate(self.simTracks.fromUE, axis = 0), "Sim_eta" :np.concatenate(self.simTracks.eta, axis = 0), "Sim_pt" :np.concatenate(self.simTracks.pt, axis = 0)})
        df.to_csv(self.output_location+"/"+csvTag+'_SimTracks.csv', index=False)
        df = pd.DataFrame({"Reco_fromSUEP" : np.concatenate(self.reco.fromSUEP, axis = 0), "Sim_fromZ" : np.concatenate(self.reco.fromZ, axis = 0), "Sim_fromISR" : np.concatenate(self.reco.fromISR, axis = 0), "Sim_fromPU" : np.concatenate(self.reco.fromPU, axis = 0), "Sim_fromUE" : np.concatenate(self.reco.fromUE, axis = 0), "Sim_eta" :np.concatenate(self.reco.eta, axis = 0), "Sim_pt" :np.concatenate(self.reco.pt, axis = 0)})
        df.to_csv(self.output_location+"/"+csvTag+'_RecoTracks.csv', index=False)
        #df = pd.DataFrame({"preShowerParton" : np.concatenate(self.preShowerParton[0], axis = 0), "eta" : np.concatenate(self.preShowerParton[1], axis = 0), "pdgID" : np.concatenate(self.preShowerParton[2], axis = 0), "status" : np.concatenate(self.preShowerParton[3], axis = 0), "motherPDG" : np.concatenate(self.preShowerParton[4], axis = 0), "pt" : np.concatenate(self.preShowerParton[5], axis = 0)})
        #df.to_csv(self.output_location+"/"+csvTag+'_preShowerParton.csv', index=False)
        ...
        if self.doTracks:
            ...
            for prop in ["pt", "eta", "phi", "mass", "d0", "d0Err", "dz", "dzErr", "puppiWeight", "puppiWeightNoLep", "vtxChi2", "fromPV", "pvAssocQuality", "fromSUEP"]:
              #print(ak.fill_none(ak.pad_none(getattr(self.tracks, prop),  maxntracks, axis=1, clip=True), -999.)[0][0])
            ...
            if self.isSpherable:
                if self.doClusters and self.isClusterable:
                    ...
                    print('Doing boosted sphericities')
