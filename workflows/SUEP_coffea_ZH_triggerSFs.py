...
class SUEP_cluster(processor.ProcessorABC):
    ...
    def doTriggerSFs(self, electrons, muons):
        ...
        if self.do_syst:
            ...
            for i in range(len(electrons)):
                if len(electrons[i]) == 2:
                    if(electrons[i].pt[1]>200 or electrons[i].pt[0]>200):
                        central_vals.append(1.0)
                        down_vals.append(0.0)
                        up_vals.append(0.0)
                    else:
                        central_vals.append(ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf","Electron", electrons[i].pt[1], electrons[i].pt[0]))
                        down_vals.append(ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf-down","Electron", electrons[i].pt[1], electrons[i].pt[0]))
                        up_vals.append(ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf-up","Electron", electrons[i].pt[1], electrons[i].pt[0]))
                elif(len(muons[i]) == 2):
                    if(muons[i].pt[1]>200 or muons[i].pt[0]>200):
                        central_vals.append(1.0)
                        down_vals.append(0.0)
                        up_vals.append(0.0)
                    else:
                        central_vals.append(ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf", "Muon", muons[i].pt[1], muons[i].pt[0]))
                        up_vals.append(ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf-up", "Muon", muons[i].pt[1], muons[i].pt[0]))
                        down_vals.append(ceval['UL-PtPt-Trigger-SFs'].evaluate(year,"sf-down", "Muon", muons[i].pt[1], muons[i].pt[0]))
            SF['central'] = ak.Array(central_vals)
            SF['up'] = ak.Array(up_vals)
            SF['down'] = ak.Array(down_vals)
        else:
            for i in range(len(electrons)):
                if len(electrons[i] == 2):
                    if(electrons[i].pt[1]>200 or electrons[i].pt[0]>200):
    ...
    def doAllPlots(self, channel, debug=True):
        ...
        if self.isMC:
           ...
           out["triggerSFs"] = self.triggerSFs["central"][:]
        ...
        if self.var == "" and self.do_syst:
           ...
           out["triggerSFs_up"]          = self.triggerSFs["up"]
           out["triggerSFs_down"]        = self.triggerSFs["down"]
